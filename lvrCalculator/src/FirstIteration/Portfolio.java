...
public class Portfolio {
	...
	/*	ArrayList<LoanAccount> removees = new ArrayList<LoanAccount>();
		for (LoanAccount ln: portfolio) {
			if (ln.getAccNum()==lnAcct){
				removees.add(ln);
			}
		}
		for (int i=0;i<removees.size();i++){
			portfolio.remove(removees.get(i));
		}*/
	}
